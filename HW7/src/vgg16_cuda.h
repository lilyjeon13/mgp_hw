...
private:
    ...
    int*    d_predict_cuda;   // [batch];
     ...
     // Function
    void gpu_normalize(const uint8_t* const image, float* input);
    void gpu_pad(float* input, float* input_padded,
                 int B, int C, int H, int W, int P);
    void gpu_relu(float* feature_map, int size);
    void gpu_conv(float* input, float* output, float* weight, float* bias,
              int B, int H, int W, int IC, int OC, int K);
    void gpu_pool(float* input, float* output,
         int B, int C, int H, int W);
    void gpu_fc(float* input, float* output, float* weight, float* bias,
         int B, int IC, int OC);
    void gpu_softmax(float* input, int* output, int B, int size);
    // P gpu_int Funtions for debug
    void gpu_print_output(float* data) {
